...
export const passwordInputAssertionBlockGenerator: OperationBlockGenerator = {
  name: 'password-input-assertion',
  canBuild: context => context.assertion?.assertionKind === 'toBePasswordInput'
  ...
  build: context => {
    ...
    if (!assertion || assertion.assertionArgumentText === null)
      throw new Error('passwordInputAssertionBlockGenerator requires a parsed password input assertion.');

    return `const ${context.targetVariable} = ${assertion.targetExpression};
await ${RECORDING_SESSION_NAME}.assertPasswordInput({
  index: ${context.index},
